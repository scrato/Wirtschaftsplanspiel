...
public class EmployeePanel extends TypedPanel {
	...
	public EmployeePanel() {
		...
		typeListEmploy.addItemListener(new costItemListener(anzahlEmploy,
				typeListEmploy, kostenEmploy));
		anzahlEmploy.addFocusListener(new costFocusListener(anzahlEmploy,
				typeListEmploy, kostenEmploy));
		employButton
				.addActionListener(new employ(anzahlEmploy, typeListEmploy));
		...
		anzahlDismiss.addFocusListener(new costFocusListener(anzahlDismiss,
				typeListDismiss, kostenDismiss));
		typeListDismiss.addItemListener(new costItemListener(anzahlDismiss,
				typeListDismiss, kostenDismiss));
		dismissButton.addActionListener(new dismiss(anzahlDismiss,
				typeListDismiss));
			...
			if (anzahl > 0) {
				if (((String) typeField.getSelectedItem()).equals("Verwaltung")) {
					...
					while (i < anzahl) {
						try {
							CompanyController
									.dismissSb(EmployeeType.Verwaltung);
							...
							JOptionPane
									.showMessageDialog(new JFrame(),
											"Keine Mitarbeiter von diesem Typ angestellt.");
							e1.printStackTrace();
							break;
						}
						i++;
					...
					while (i < anzahl) {
						try {
							CompanyController
									.dismissSb(EmployeeType.Produktion);
						} catch (UserCanNotPayException e1) {
							// TODO Auto-generated catch block
							JOptionPane.showMessageDialog(new JFrame(),
									"Nicht genügend liquide Mittel.");
	...
	private class costFocusListener implements FocusListener {
		...
		public costFocusListener(JTextField anzahl, JComboBox type,
				JTextField kostenOutput) {
			this.anzahlField = anzahl;
			this.typeField = type;
	...
	private class costItemListener implements ItemListener {
		...
		public costItemListener(JTextField anzahl, JComboBox type,
				JTextField kostenOutput) {
			this.anzahlField = anzahl;
			this.typeField = type;
			this.kostenOutput = kostenOutput;
